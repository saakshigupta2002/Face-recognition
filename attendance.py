...
def takeImage():
    ...
    if name.isalpha():
        ...
        detector = cv2.CascadeClassifier(os.path.join(cv2.data.haarcascades, harcascadePath))
        print(detector)
        sampleNum = 0
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite(r'R:\\STUDENT-ATTENDANCE-USING-FACIAL-RECOGNITION-SYSTEM-OPENCV-master\\TrainingImages\\' + name + "." + Id + '.' + str(sampleNum) + ".jpg",
                            gray[y:y + h, x:x + h])
                print("Image Saved")
        ...
        with open(r'R:\\STUDENT-ATTENDANCE-USING-FACIAL-RECOGNITION-SYSTEM-OPENCV-master\\studentDetailss.csv', 'a+') as csvFile:
...
def trainImage():
    ...
    harcascadePath = "haarcascade_frontalface_default.xml"
    detector = cv2.CascadeClassifier(os.path.join(cv2.data.haarcascades, harcascadePath))
    faces, Id = getImagesAndLabels(r'R:\\STUDENT-ATTENDANCE-USING-FACIAL-RECOGNITION-SYSTEM-OPENCV-master\\TrainingImages')
...
def trackImage():
    ...
    harcascadePath = "haarcascade_frontalface_default.xml"
    faceCascade = cv2.CascadeClassifier(os.path.join(cv2.data.haarcascades, harcascadePath))
    df = pd.read_csv(r'R:\\STUDENT-ATTENDANCE-USING-FACIAL-RECOGNITION-SYSTEM-OPENCV-master\\studentDetailss.csv')
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if conf < 60:
                ...
                with open(r'R:\\STUDENT-ATTENDANCE-USING-FACIAL-RECOGNITION-SYSTEM-OPENCV-master\\AttendanceFile.csv', 'a+') as csvFile2:
                ...
                if conf > 65:
                    ...
                    print(os.listdir("ImagesUnknown"))
